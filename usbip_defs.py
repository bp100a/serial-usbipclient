...
class BaseProtocolPacket:  # pylint: disable = too-few-public-methods
    ...
    def __init__(self, version: int = 0x0111, command: int = 0, status: int = 0):
        ...
        if not BaseProtocolPacket.fmt:
            BaseProtocolPacket.fmt = BaseProtocolPacket.endianness + "".join([BaseProtocolPacket.format[item][0]
                                                                              for item in BaseProtocolPacket.format])
            BaseProtocolPacket.args = [BaseProtocolPacket.format[item][1] for item in BaseProtocolPacket.format]

    def packet(self) -> bytes:
        """Turn this structure into binary representation"""
        packet_bytes = struct.pack(self.fmt, *[getattr(self, item) for item in self.args])
        return packet_bytes
    ...
    def new(cls, data: bytes):
        ...
        try:
            ...
            raise ValueError(f"error parsing binary data:'{data.hex()}', expected(format):'{cls().fmt}'") from s_error
